 ...
 * @since 1.5
 */
class HTMLTemplateOrderConfirmation extends HTMLTemplate
{
    public $order;
    public $shop_address;
     ...
     */
    public function __construct(Order $order, $smarty, $bulk_mode = false)
    {
        $this->order = $order;
        $this->smarty = $smarty;

        $this->shop_address = $this->getCurrentFormattedShopAddress((int)$this->order->id_shop);
    ...
    public function getContent()
    {
        ...
        $order_payment = OrderPayment::getByOrderReference($this->order->reference);
        ...
        $tpls = array(
            'style_tab' => $this->smarty->fetch($this->getTemplate('orderconfirmation.style-tab')),
            'addresses_tab' => $this->smarty->fetch($this->getTemplate('orderconfirmation.addresses-tab')),
            'summary_tab' => $this->smarty->fetch($this->getTemplate('orderconfirmation.summary-tab')),
            'product_tab' => $this->smarty->fetch($this->getTemplate('orderconfirmation.product-tab')),
            'tax_tab' => $this->getTaxTabContent(),
            'payment_tab' => $this->smarty->fetch($this->getTemplate('orderconfirmation.payment-tab')),
            'note_tab' => $this->smarty->fetch($this->getTemplate('orderconfirmation.note-tab')),
            'total_tab' => $this->smarty->fetch($this->getTemplate('orderconfirmation.total-tab')),
            'shipping_tab' => $this->smarty->fetch($this->getTemplate('orderconfirmation.shipping-tab')),
        );
    ...
    public function getTaxTabContent()
    {
        ...
        $data = array(
            ...
            'display_tax_bases_in_breakdowns' => !$this->order->useOneAfterAnotherTaxComputationMethod(),
            ...
            'wrapping_tax_breakdown' => $this->order->getWrappingTaxesBreakdown(),
        ...
        return $this->smarty->fetch($this->getTemplate('orderconfirmation.tax-tab'));
    }
    ...
    protected function getTaxBreakdown()
    {
        $breakdowns = array(
            ...
            'wrapping_tax' => $this->order->getWrappingTaxesBreakdown(),
        );
    ...
    */
    ...
    /**
     * Returns the orderconfirmation template associated to the country iso_code
     *
    ...
    protected function getTemplate($template_name)
    {
        ...
        } elseif (file_exists($overridden_template)) {
    ...
    {
        return 'order_confirmations.pdf';
    }
    ...
    {
        return $this->prefix . $this->order->reference . '.pdf';
    }
    ...
    private function getProductTaxesBreakdown()
    {
        $sum_composite_taxes = $this->order->useOneAfterAnotherTaxComputationMethod();
    ...
    public function getShippingTaxesBreakdown()
    {
        ...
        $carrier = new Carrier((int)$this->order->id_carrier);
        $tax_calculator = $carrier->getTaxCalculator(new Address($this->order->id_address_invoice));
        $total_taxes = $tax_calculator->getTaxesAmount($this->order->total_shipping_tax_excl);
